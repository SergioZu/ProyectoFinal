...
public interface NotaRepository extends CrudRepository<NotaEntity, Integer>{
	@Query(value = "select new com.sergio.colegio.dtos.NotaDTO (n.id, a.id, a.nombre, asig.id, asig.nombre, n.nota, n.fecha) "
			+ "FROM com.sergio.colegio.entities.NotaEntity n JOIN com.sergio.colegio.entities.AlumnoEntity a ON n.alumnos.id = a.id "
			+ "JOIN com.sergio.colegio.entities.AsignaturaEntity asig ON n.asignaturas.id = asig.id "
